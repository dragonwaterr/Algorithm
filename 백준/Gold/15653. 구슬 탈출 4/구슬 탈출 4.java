...
public class Main {
    ...
    static int[][] graph;
    static HashSet<Integer> memeory;
    ...
    static int bfs() {
        ...
        while(!redDeque.isEmpty()) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                // 빨간 구슬만 구멍에 들어갔다면 종료
                if(redIsInHole) return redTempPos[2]+1;

                int curRed = 100 * (redTempPos[0] * 10 + redTempPos[1]);
                int curBlue = blueTempPos[0] * 10 + blueTempPos[1];
                if(memeory.contains(curRed + curBlue)) continue;
                memeory.add(curRed + curBlue);
