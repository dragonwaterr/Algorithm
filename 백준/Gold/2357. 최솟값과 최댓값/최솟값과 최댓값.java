...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = firstLeaf; i < firstLeaf + n; i++) {
            maxSegtree[i] = minSegtree[i] = Integer.parseInt(br.readLine());
        }
        ...
        for (int i = firstLeaf - 1; i > 0; i--) {
            ...
            maxSegtree[i] = Math.max(maxSegtree[2 * i], maxSegtree[2 * i + 1]);
        }
        ...
        for (int i = 0; i < q; i++) {
            ...
            if(from == to) {
                sb.append(maxSegtree[from + firstLeaf - 1]).append(" ")
                    .append(maxSegtree[from + firstLeaf - 1]).append("\n");
                continue;
            }
    ...
    static void answerQuery(int from, int to) {
        ...
        while (from <= to) {
            if (from % 2 == 1) {
                ...
                max = Math.max(max, maxSegtree[from]);
                from++;
            }
            if (to % 2 == 0) {
                min = Math.min(min, minSegtree[to]);
                max = Math.max(max, maxSegtree[to]);
                to--;
            }
            from /= 2;
            to /= 2;
